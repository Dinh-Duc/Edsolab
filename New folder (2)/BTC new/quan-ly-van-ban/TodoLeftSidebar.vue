          ...
          <vue-perfect-scrollbar :settings="perfectScrollbarSettings">
            <div class="menu-filter-docs">
              ...
              <div
                ...
                @click="chooseFilterAll(headerLeftmenu)"
              >
                <div
              ...
              <!-- Văn bản đến -->
              <div class="filter-goto-elm">
                <!-- Tags -->
                <div class="title-doc-goto">
                  <b>
                    {{ goDocs.Name }}
                  </b>
                </div>
                <div class="sub-filter-docs">
                  <ul>
                    <li
                      v-for="(filter, indexChild) in goDocs.Child"
                      :key="indexChild"
                      @click="chooseFilterAll(filter)"
                    >
                      <div
                        class="none-active-filter"
                        :class="{ 'active-tab': NameSlug === filter.NameSlug }"
                      />
                      <div class="text-filter-elm">
                        <div>
                          ...
                          <span
                            class="text-elm-filter"
                            :class="{
                              'active-name': NameSlug === filter.NameSlug,
                            }"
                          >{{ filter.Name }}</span>
                        </div>
                        <b-badge class="align-self-center">
              ...
              <div class="filter-goto-elm">
                ...
                <div class="sub-filter-docs">
                  <ul>
                    <li
                      v-for="(filter, indexChild) in toDocs.Child"
                      :key="indexChild"
                      @click="chooseFilterAll(filter)"
                    >
                      <div class="none-active-filter" />
                      <div class="text-filter-elm">
                        <div>
                          <svg
                            width="12"
...
export default {
  ...
  data() {
    return {
      ...
      goDocs: {
        ...
        Child: [
          ...
          },
          {
            ...
            Total: 0,
            NameSlug: 'VBDEN_chuaXuLyTrongHan',
            TinhTrang: 0,
            TrangThai: 0,
          ...
          },
          {
            ...
            Total: 0,
            NameSlug: 'VBDEN_dangXuLyTrongHan',
            TinhTrang: 1,
            TrangThai: 0,
      ...
      toDocs: {
        Name: 'Văn bản đi',
        Child: [
          { Name: 'Tổng quan', Total: 407, NameSlug: 'VBDI_TONG_QUAN' },
          { Name: 'Chờ trình ký', Total: 85, NameSlug: 'VBDI_CHO_TRINH_KY' },
          { Name: 'Chờ xử lý', Total: 120, NameSlug: 'VBDI_CHO_XU_LY' },
          { Name: 'Đã xử lý', Total: 50, NameSlug: 'VBDI_DA_XU_LY' },
          { Name: 'Đã trả lại', Total: 50, NameSlug: 'VBDI_DA_TRA_LAI' },
          { Name: 'Chờ ý kiến', Total: 65, NameSlug: 'VBDI_CHO_Y_KIEN' },
          { Name: 'Chờ cấp số', Total: 37, NameSlug: 'VBDI_CHO_CAP_SO' },
        ],
      },
      IsAllTab: true,
      NameSlug: '',
  ...
  methods: {
    chooseFilterAll(data) {
      ...
      if (data.NameSlug.includes('VBDEN_')) {
        model.DocTypeId = 1
      } else if (data.NameSlug.includes('VBDI_')) {
        model.DocTypeId = 2
      } else {
        model.DocTypeId = 0
      }
        ...
        COMMON_GET(
          ...
          if (res) {
            for (let index = 0; index < this.goDocs.Child.length; index += 1) {
              if (this.goDocs.Child[index].NameSlug === 'VBDEN_tongQuan') {
                // this.goDocs.Child[index].Total = res.daXuLy + res.dangXuLy + res.choXuLy
                this.goDocs.Child[index].Total = 378
              } else if (
                this.goDocs.Child[index].NameSlug === 'VBDEN_chuaXuLyQuaHan'
              ) {
                this.goDocs.Child[index].Total = 80
              } else if (
                this.goDocs.Child[index].NameSlug === 'VBDEN_chuaXuLyTrongHan'
              ) {
                // this.goDocs.Child[index].Total = res.choXuLy
                this.goDocs.Child[index].Total = 30
              } else if (
                this.goDocs.Child[index].NameSlug === 'VBDEN_daXuLyQuaHan'
              ) {
                this.goDocs.Child[index].Total = 65
              } else if (
                this.goDocs.Child[index].NameSlug === 'VBDEN_daXuLyTrongHan'
              ) {
                this.goDocs.Child[index].Total = 78
              } else if (
                this.goDocs.Child[index].NameSlug === 'VBDEN_dangXuLyQuaHan'
              ) {
                this.goDocs.Child[index].Total = 50
              } else if (
                this.goDocs.Child[index].NameSlug === 'VBDEN_dangXuLyTrongHan'
              ) {
                // this.goDocs.Child[index].Total = res.dangXuLy
                this.goDocs.Child[index].Total = 75
              }
            }
          }
        })
