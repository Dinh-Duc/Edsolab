<template>
  <div class="form-calendar-dn flex">
    <div>
      <b-row>
        <b-col cols="12">
          <b-form-group :label="isEdit ? '' : 'Nhập tiêu đề *'">
            <b-form-input
              v-model="value.title"
...
export default {
  ...
  computed: {
    step: {
      ...
      set(newValue) {
        this.$emit('update:stepForm', newValue)
      },
    },
    configCalendar() {
      const day = new Date()
  ...
  watch: {
    dayRepeat: {
      handler(val) {
        if (this.calendarPreviewApi) {
          ...
          eventDraft.setProp(
            'title',
            this.setTitleEventAllday(eventDraft.start, val),
          )
        }
        if (val > 1) {
          this.dataform.dateTo = moment(this.dataform.dateFrom)
            .add(val, 'days')
            .format('YYYY-MM-DD')
        }
      },
    ...
    value: {
      deep: true,
      handler(val, oldVal) {
        this.dataform.dateTo = val.dateFrom
        this.calendarPreviewApi.gotoDate(this.dataform.dateFrom)
      },
    },
  },
  mounted() {
  ...
  methods: {
    ...
    addEventDraft() {
      ...
      // const endTime = moment(currentTime).add(1, 'hours')
      this.dataform.dateFrom = moment(this.dateFrom).format('YYYY-MM-DD')
      if (!this.isEdit) {
      ...
      const defaultEvent = {
        id: 'draft-event',
        title: moment(this.convertDateWithFurture(currentTime, this.dateFrom))
          .locale('vi')
          .format('dd D/MM/YYYY'),
      ...
      if (this.isEdit) {
        defaultEvent.start = this.value.dateTimeFrom
        defaultEvent.end = this.value.dateTimeTo
      }
      if (
    ...
    setTitleEventAllday(daystart, dayrepeat) {
      if (dayrepeat > 1) {
        return `${moment(daystart)
          .locale('vi')
          .format('dd D/MM/YYYY')} - ${moment(daystart)
          .locale('vi')
          .add(dayrepeat, 'days')
          .format('dd D/MM/YYYY')}`
      }
