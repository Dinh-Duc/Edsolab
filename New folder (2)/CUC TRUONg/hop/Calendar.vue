<template>
  <div
    class="app-calendar overflow-hidden border shadow-none mb-0 hop-calendar"
  >
    <div class="flex">
      <!-- Sidebar -->
      <div
      ...
      >
        <div class="h-full flex flex-column">
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  v-model="keySearchPositionHeader"
                  placeholder="Tìm kiếm"
                  class="border-none flex-1"
                  @keyup.13="actionSearchPositionHeader"
                >
              </div>
              <div
          ...
          <div class="summary-calendar d-flex align-items-center space-x-28">
            <div
              v-for="(item, index) in MeetingCounter"
              :key="index"
              class="col-3 p-0 m-0"
            >
              <div class="summary-calendar__item m-1">
            ...
            >
              ...
              <template #eventContent="{ event, view }">
                ...
                >
                  ...
                  >
                    <span v-html="actionThoiGian(event.start, event.end)" />
                  </div>
                  <div
                    class="row-noidung"
                    @click="viewEvent(event)"
                  >
                    {{ event.extendedProps.data.title }}
                  </div>
                  <div
                    class="row-thanhphan"
                    @click="viewEvent(event)"
                  >
                    {{ event.extendedProps.data.canBoChuTriInfo.hoTen }} -
                    {{ event.extendedProps.data.canBoChuTriInfo.tenChucVu }}
                  </div>
                  <div
                    class="row-loai justify-content-center align-items-center"
                  >
                    <b-button
                      v-if="event.extendedProps.data.linkTrucTuyen"
                      v-ripple.400="'rgba(255, 255, 255, 0.15)'"
                      class="w-100"
                      variant="warning"
                      @click="
                        openLinkTrucTuyen(
                          event.extendedProps.data.linkTrucTuyen,
                        )
                      "
                    >
                      Trực tuyến
                  ...
                  <div
                    class="row-trangthai d-flex justify-content-center align-items-center"
                  >
                    <b-badge
                      :variant="
                        actionColorStatus(event.extendedProps.data.status)
                      "
                    >
                      {{ event.extendedProps.data.statusName }}
                    </b-badge>
                  </div>
                  <!-- {{ event }} {{ view }} -->
                </div>
          ...
          <div
            v-if="typeView === 'list'"
            class="d-flex justify-content-center pt-1 pb-0 mb-0"
          >
            <b-pagination
              v-model="pageIndex"
              :total-rows="EventPaging.totalCount"
              :per-page="EventPaging.pageSize"
              @change="actionChange"
            />
          </div>
        </div>
...
export default {
      ...
      return {
        ...
        headerToolbar: {
          ...
          center: '',
            ...
            'list,grid listDay,listWeek,listMonth,timeGridDay,timeGridWeek,dayGridMonth',
        },
        buttonText: {
          today: 'Hôm nay',
          month: 'Tháng',
        ...
        customButtons: {
          list: {
            text: '',
            click() {
              document.querySelector('.fc-timeGridDay-button').style.display =
                'none'
              document.querySelector('.fc-timeGridWeek-button').style.display =
                'none'
              document.querySelector('.fc-dayGridMonth-button').style.display =
                'none'
              document.querySelector('.fc-listDay-button').style.display =
                'block'
              document.querySelector('.fc-listWeek-button').style.display =
                'block'
              document.querySelector('.fc-listMonth-button').style.display =
                'block'
  ...
  created() {
    ...
    this.$store.dispatch('hop/getListDonVi')
    this.renderDashboard()
  },
  mounted() {
    this.idQueryFromLich = this.$route.query.id
  ...
  methods: {
    actionSearchPositionHeader() {
      this.paramPayload.KeyWord = this.keySearchPositionHeader
      this.paramPayload.PageIndex = 1
      this.$store.dispatch('hop/getListEvents', this.paramPayload)
      this.keySearchPositionHeader = ''
    },
    renderDashboard() {
      this.getMeetingCountDashboard()
    },
    getMeetingCountDashboard() {
      const payload = {
        donviId: this.UserData.donViTrucThuoc.id.toString(),
        canboId: this.UserData.id.toString(),
      }
      this.$store.dispatch('hop/getMeetingCount', payload)
    },
    getCanBoDangKyId() {
      const info = JSON.parse(localStorage.getItem(USER_DATA))
      this.paramPayload.CanBoId = info.id.toString()
      this.paramPayload.KeyWord = null
      this.keySearchPositionHeader = ''
      this.$store.dispatch('hop/getListEvents', this.paramPayload)
    },
    actionChange(val) {
      this.paramPayload.PageIndex = val
      this.pageIndex = val

      if (this.isActiveLeftSideBarHop === 'personal') {
        this.paramPayload.Loai = -1
        this.paramPayload.DonViId = null
        this.paramPayload.CanBoId = JSON.parse(
          localStorage.getItem(USER_DATA),
        ).id.toString()
      } else if (this.isActiveLeftSideBarHop === 'unit') {
        this.paramPayload.Loai = 1
        this.paramPayload.DonViId = JSON.parse(
          localStorage.getItem(USER_DATA),
        ).donViTrucThuoc.id.toString()
        this.paramPayload.CanBoId = null
      } else {
        this.paramPayload.Loai = 1
        this.paramPayload.DonViId = this.isActiveLeftSideBarHop
        this.paramPayload.CanBoId = null
      }
      this.$store.dispatch('hop/getListEvents', this.paramPayload)
    },
    ...
    actionThoiGian(start, end) {
      ...
      if (tmp_start[0] == tmp_end[0]) {
        // nếu cùng ngày
        return `${tmp_start[0]}<br>${tmp_start[1]} - ${tmp_end[1]}`
      }
      return `${tmp_start[0]}<br>${tmp_start[1]} - ${tmp_end[0]} ${tmp_end[1]}`
    },
    formatDate(value) {
      if (value) {
    ...
    updateHop() {
      ;(this.paramPayload.DateStart = moment(String(this.calendarApi.getDate()))
        .subtract(0, 'months')
        .startOf('month')
        .format('YYYY-MM-DD')),
        (this.paramPayload.DateTo = moment(String(this.calendarApi.getDate()))
          .subtract(0, 'months')
          .endOf('month')
          .format('YYYY-MM-DD')),
        this.$store.dispatch('hop/getListEvents', this.paramPayload)
    },
    getDataByDate(from, to) {
      this.paramPayload.DateStart = moment(from).format('YYYY-MM-DD')
      this.paramPayload.DateTo = moment(to).format('YYYY-MM-DD')
      this.$store.dispatch('hop/getListEvents', this.paramPayload)
    },
    deleteEvent(event) {
      this.$bvModal
    ...
    viewEvent(event) {
      ...
      //   },
      // })
      // console.log(event.extendedProps.data.id)
      this.eventDetail = event.extendedProps.data.id
